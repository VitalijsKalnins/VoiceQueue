...
class MatchmakingService:
    def __init__(self, similarity_alpha: float):
            ...
            for idy, profileB_ent in enumerate(profileB.entities):
                ...
                if (ent_similarity >= self.SIMILARITY_ALPHA):
                    ...
                    if (profileA_ent.type == profileB_ent.type):
                        ...
                        score += round(avg_sentiment * ent_similarity_normalized)
                    else:
                        delta_sentiment = abs(profileA_ent.sentiment - profileB_ent.sentiment)
                        score -= round(delta_sentiment * ent_similarity_normalized)
    ...
    def matchmake(self, users: List[int]) -> Dict:
        res: Dict = {}
        ...
        user_to_profile = ProfileService.get_profiles(users)
...
service = MatchmakingService(similarity_alpha = 0.25)